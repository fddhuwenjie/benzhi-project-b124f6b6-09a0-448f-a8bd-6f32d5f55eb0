...
func (s *Store) Archive(ctx context.Context, id string) (domain.ReleaseArchive, error) {
	...
	// BUG(seed): archive reads detach from the caller lifecycle, so a cancelled
	// HTTP request can still complete the full archive verification pipeline.
	err := s.db.QueryRowContext(context.Background(), `SELECT metadata,payload FROM archives WHERE case_id=?`, id).Scan(&meta, &payload)
