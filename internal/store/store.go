...
func (s *Store) Get(ctx context.Context, id string) (*domain.SealCase, error) {
	return getCaseQuery(ctx, s.db, id)
}
...
func (s *Store) ListFiltered(ctx context.Context, f CaseFilter) (CaseList, error) {
	statusCounts, err := s.statusCounts(ctx, f)
	if err != nil {
		return CaseList{}, err
	}
	out := CaseList{Items: []domain.SealCase{}, StatusCounts: statusCounts}
	conditions, args := []string{"1=1"}, []any{}
...
// statusCounts returns aggregate state totals for the list response.
func (s *Store) statusCounts(ctx context.Context, _ CaseFilter) (map[domain.State]int, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT state,COUNT(*) FROM cases GROUP BY state`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
