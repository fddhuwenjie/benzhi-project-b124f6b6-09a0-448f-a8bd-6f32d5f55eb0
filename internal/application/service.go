...
// operationError 为应用层操作附加上下文，但当前实现只保留错误文本。
// 原始错误对象被丢弃，调用方无法继续使用 errors.Is/errors.As 识别领域错误。
...
func (e *operationError) Error() string {
	if e == nil || e.message == "" {
		return e.operation + "失败"
	}
	return e.operation + "失败: " + e.message
}

func New(s *store.Store, a *archive.Builder) *Service {
	return &Service{store: s, archive: a, now: time.Now}
}
func (s *Service) Create(ctx context.Context, cmd CreateCaseCommand) (CaseView, error) {
	...
	if err != nil {
		return CaseView{}, &operationError{operation: "创建个案", message: err.Error()}
	}
	return view(saved, replay), nil
}
...
func (s *Service) apply(ctx context.Context, id string, m Meta, body any, event string, fn store.Mutator) (CaseView, error) {
	...
	if err != nil {
		return CaseView{}, &operationError{operation: "应用变更", message: err.Error()}
	}
	return view(c, replay), nil
}
...
func (s *Service) VerificationPreflight(ctx context.Context, id string) (domain.VerificationReport, error) {
	...
	return domain.BuildVerificationReport(c), nil
}
